...
function onetone_CountTail($number){  
  ...
  global $onetone_social_icons;
  
  

$onetone_social_icons = array(
...
function onetone_section_options($id,$index=''){
	global $onetone_option_args;
	$hide_section = '';
	$content_model = '0';
	extract($onetone_option_args);
		
	if(  $id >= $o_section_num && $o_section_num >0  )
		$hide_section = 1;
	if(  $o_section_num <=0 && $id > 8 ){
		$hide_section  = 1;
		$content_model = 1;
	}
	
	
	if(!isset($section_title[intval($id)])){ 
		$section_title[intval($id)] = "";
	}
			 
	if(!isset($section_subtitle[intval($id)])){
		$section_subtitle[intval($id)] = "";
	}

	if(!isset($section_color[intval($id)])){
		$section_color[intval($id)] = "";
	}
			
	if(!isset($section_menu[intval($id)])){
		$section_menu[intval($id)] = "";
	}
		
	if(!isset($section_background[intval($id)])){
		$section_background[intval($id)] = array('color' => '','image' => '','repeat' => '','position' => '','attachment'=>'');
	}
	...
	if(!isset($background_size[intval($id)])){
		$background_size[intval($id)] = "";
	}
			
	if(!isset($parallax_scrolling[intval($id)])){
		$parallax_scrolling[intval($id)] = "";
	}
			
	if(!isset($full_width[intval($id)])){
		$full_width[intval($id)] = "";
	}
		
	if(!isset($section_css_class[intval($id)])){
		$section_css_class[intval($id)] = "";
	}
			
	if(!isset($section_content[intval($id)])){
		$section_content[intval($id)] = "";
	}
			
	if(!isset($section_slug[intval($id)])){ 
		$section_slug[intval($id)] = "";
	}
			 
	if(!isset($text_align[intval($id)])){ 
		$text_align[intval($id)] = "";
	}
			 
	if(!isset($section_padding[intval($id)])){ 
		$section_padding[$id] = "";
	}
		
	if(!isset($section_padding[intval($id)])){ 
		$section_padding[intval($id)] = "";
	}
		
	if(!isset($section_title_typography_defaults[intval($id)])){ 
		$section_title_typography_defaults[intval($id)] = array('size'  => '36px','face'  => '','style' => 'normal','color' => '#666666' );
	}
		   
	if(!isset($section_subtitle_typography_defaults[intval($id)])){ 
		$section_subtitle_typography_defaults[intval($id)] = array('size'  => '14px','face'  => '','style' => 'normal','color' => '#666666' );
	}  
		   
	if(!isset($section_content_typography_defaults[intval($id)])){ 
		$section_content_typography_defaults[intval($id)] = array('size'  => '14px','face'  => '','style' => 'normal','color' => '#666666' );
		}
				
	$section_name = isset($theme_options[ 'section_title_'.$id])?$theme_options['section_title_'.$id]:''; 
	$menu_title   = isset($theme_options[ 'menu_title_'.$id])?$theme_options['menu_title_'.$id]:''; 
	$section_name = $section_name?$section_name:$menu_title;
	$section_name = $section_name? ' ('.$section_name.')':'';
	

	$section_name = onetone_get_section_name($id) .' '. $section_name;
	
	//if( $index !='' )
		//$section_name = str_replace($i+1,$index,$section_name);
	
	$add_section_class  = '';
	$delete_section_btn = '';
	if(!is_numeric($id)){
		$add_section_class = ' home-new-section';
		$delete_section_btn = '<a data-section="'.$id.'" id="delete-section-'.$id.'"  title="'.__('Delete this section.', 'onetone').'" class="delete-section-button fa fa-times"></a>';
		
	}
		
	$options[] = array('name' => '','id' => 'section_group_start_'.$id.'','type' => 'start_group','class'=>'home-section group_close'.$add_section_class);
		
	$options[] =   	 array(
		'id'          => 'sections_titled_'.$id,
		'name'       => $section_name .' <span id="accordion-group-section-'.$id.'" class="fa fa-plus"></span>'.$delete_section_btn,
		'desc'        => '',
		'std'         => '',
		'type'        => 'textblock-titled',
		'rows'        => '',
		'class'       => 'section-accordion close accordion-group-title-section-'.$id
      );
	...
	$options[] = array(
		...
		'id' => 'section_hide_'.$id,
		 'type' => 'checkbox',
		'class'=>'section-item accordion-group-section-'.$id,
	...
	$options[] = array(
		...
		'id' => 'section_background_'.$id.'',
		'std' => $section_background[intval($id)],
		'type' => 'background' ,
		'class'=>'section-item accordion-group-section-'.$id,
	...
	$options[] = array(
		...
		'class'=>'mini section-item accordion-group-section-'.$id,
	...
	$options[] = array(
		...
		'type' => 'text',
		'std'=>$section_css_class[intval($id)],
		'class'=>'section-item accordion-group-section-'.$id,
	...
	$options[] = array(
		...
		'id'   => "section_title_typography_".$id,
		'std'  => $section_title_typography_defaults[intval($id)],
		'type' => 'typography',
	...
	$options[] = array(
		...
		'id'   => "section_subtitle_typography_".$id,
		'std'  => $section_subtitle_typography_defaults[intval($id)],
		'type' => 'typography',
		'options' => $typography_options ,
		'class'=>'section-item accordion-group-section-'.$id
		);	
	...
	$options[] = array(
		...
		'id'   => "section_content_typography_".$id,
		'std'  => $section_content_typography_defaults[intval($id)],
		'type' => 'typography',
		'options' => $typography_options ,
		'class'=>'section-item accordion-group-section-'.$id
		);
	...
	$options[] = array(
		...
		'std' => $content_model,
		'class' => 'section-content-model section-item accordion-group-section-'.$id,
		'type' => 'radio',
	...
	$options[] = array(
		...
		'type' => 'text',
		'std'=> $section_subtitle[intval($id)],
		'class'=>'content-model-0 section-item accordion-group-section-'.$id,
	...
	switch( intval($id) ){
			...
			$options[] = array(
		     ...
		     $options[] = array(
			  ...
			  $options[] = array(
						  ...
						  'id'   => "section_social_icon_".$id."_".$s,
			...
			for( $t=0; $t<8; $t++ ){
				...
				for($k=0;$k<4;$k++):
					...
					$options[] = array(
						  'id' => 'section_icon_link_'.$id.'_'.$t.'_'.$k,
...
<p>Morbi rutrum, elit ac fermentum egestas, tortor ante vestibulum est, eget scelerisque nisl velit eget tellus. Fusce porta facilisis luctus. Integer neque dolor, rhoncus nec euismod eget, pharetra et tortor. Nulla id pulvinar nunc. Vestibulum auctor nisl vel lectus ullamcorper sed pellentesque dolor eleifend. Praesent lobortis magna vel diam mattis sagittis.Mauris porta odio eu risus scelerisque id facilisis ipsum dictum vitae volutpat. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed pulvinar neque eu purus sollicitudin et sollicitudin dui ultricies. Maecenas cursus auctor tellus sit amet blandit. Maecenas a erat ac nibh molestie interdum. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Sed lorem enim, ultricies sed sodales id, convallis molestie ipsum. Morbi eget dolor ligula. Vivamus accumsan rutrum nisi nec elementum. Pellentesque at nunc risus. Phasellus ullamcorper bibendum varius. Quisque quis ligula sit amet felis ornare porta. Aenean viverra lacus et mi elementum mollis. Praesent eu justo elit.</p>',
  ...
  <div>
	...
	$options[] = array(
		...
		'std' => '',
		'type' => 'editor',
		'class'=>'section-item content-model-1 accordion-group-section-'.$id
		);	  

	$options[] = array('name' => '','id' => 'wrapper_end','type' => 'wrapper_end','class'=>'');
	$options[] = array('name' => '','id' => 'section_group_end_'.$id.'','type' => 'end_group');
...
function optionsframework_options() {
	...
	$onetone_option_args['typography_options'] =  $typography_options;
	...
	$onetone_option_args['choices_reverse'] = $choices_reverse;
	...
	$onetone_option_args['content_model'] =  $content_model;
	...
	$home_sections = array(
		array('id'=>'0','name'=> __('Section 1 - Banner', 'onetone' )),
		array('id'=>'1','name'=> __('Section 2 - Slogan', 'onetone' )),
		array('id'=>'2','name'=> __('Section 3 - Service', 'onetone' )),
		array('id'=>'3','name'=> __('Section 4 - Gallery', 'onetone' )),
		array('id'=>'4','name'=> __('Section 5 - Team', 'onetone' )),
		array('id'=>'5','name'=> __('Section 6 - About', 'onetone' )),
		array('id'=>'6','name'=> __('Section 7 - Counter', 'onetone' )),
		array('id'=>'7','name'=> __('Section 8 - Testimonial', 'onetone' )),
		array('id'=>'8','name'=> __('Section 9 - Contact', 'onetone' )),
		array('id'=>'9','name'=> __('Section 10 - Portfolio', 'onetone' )),
		array('id'=>'10','name'=> __('Section 11 - Pricing', 'onetone' )),
		array('id'=>'11','name'=> __('Section 12 - Blog', 'onetone' )),
		array('id'=>'12','name'=> sprintf(__('Section %s', 'onetone'),13)),
		array('id'=>'13','name'=> sprintf(__('Section %s', 'onetone'),14)),
		array('id'=>'14','name'=> sprintf(__('Section %s', 'onetone'),15)),
		);
	...
	$onetone_option_args['home_sections'] = $home_sections;
	...
	$options[] = array(
		'name' => __('Anchor', 'onetone'),
		'desc' => __('Define how the video will behave once the window is resized; possible value are: "top,bottom,left,right,center"; it accept a pair of value comma separated (ex: "top,right" or "bottom,left").', 'onetone'),
		'id' => 'youtube_anchor',
		'std' => 'ceter,center',
		'class' => 'mini section-item accordion-group-youtube_video',
		'type' => 'text');
	...
	$options[] = array(
		'name' => __('Quality', 'onetone'),
		'desc' => __('Choose to set the video quality.', 'onetone'),
		'id' => 'youtube_quality',
		'std' => 'default',
		'class' => 'mini section-item accordion-group-youtube_video',
		'options' => array( 'default'=>__('Default', 'onetone'),
		...
		'type' => 'select');
	$options[] = array(
		'name' => __('Opacity', 'onetone'),
		'desc' => __('Choose to set the video opacity.', 'onetone'),
		'id' => 'youtube_opacity',
		'std' => '1',
		'class' => 'mini section-item accordion-group-youtube_video',
		'options' =>$opacity,
	...
	foreach( $onetone_home_sections as $k=>$v ){
		$video_background_section[$v['id']+1] = $v['name'];
	}
	...
	$onetone_option_args['o_section_num'] = $o_section_num; 
	...
	foreach(  $onetone_home_sections as $k => $v ){
		$section_options[] = onetone_section_options($v['id']);
	}
	...
	$o_sections = '<option value="" selected="selected">'.__('Select a Section', 'onetone').'</option>';
	for( $i=1; $i< 12; $i++ ){
		$o_sections .= '<option value="'.$home_sections[$i]['id'].'">'.$home_sections[$i]['name'].'</option>';
		}
	$options[] = array(
		'name' => '',
		'desc' =>'<select id="section-id">'.$o_sections.'</select><a id="add-section" class="btn button-primary">'.__('Add Section', 'onetone').'</a>',
		'id' => 'onetone_get_pro',
		'std' => '',
		'type' => 'info',
		'class'=>'section-item accordion-group-section-order'
